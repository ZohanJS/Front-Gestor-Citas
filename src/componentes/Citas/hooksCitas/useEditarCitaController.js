...
export function useEditarCitaController(currentCita) {
  const { push } = useHistory()
  const [auth ] = useContext(CRMContext );
  const[cita, guardarCita] = useState(currentCita);
  ...
  function EditarCita(event) {
    ...
    // console.log(cita)
    odontologoAxios.put(`/api/cita/update/${cita._id}`, cita, {
      headers: {
  ...
  return {handleChange, EditarCita, validarCita, filtroTipoCitaYSede, filtroOndont, DatosTipoCita, horarios, odontologos,cita}
}
